...
// --- Placeholder downloads (no binaries in repo) ---
const placeholderFiles = {
  'bts-pdf': {
    filename: 'bts-sisr-parcours.pdf',
    mime: 'text/plain',
    content: [
      'BTS SISR — Parcours et compétences',
      '',
      'Ce fichier texte est une version provisoire. Le PDF officiel n\'est pas inclus dans le dépôt pour éviter d\'ajouter des binaires.',
      'Ajoutez le document final dans le dossier assets (non versionné) ou mettez à jour le bouton pour pointer vers un hébergement externe.',
    ].join('\n'),
  },
  'synthesis-excel': {
    filename: 'tableau-synthese.xlsx',
    mime: 'text/plain',
    content: [
      'Tableau de synthèse BTS SIO (version texte temporaire)',
      '',
      'Le fichier Excel officiel sera intégré depuis un emplacement externe. Cette note rappelle que les binaires ne sont pas suivis dans le dépôt.',
      'Remplacez le bouton par un lien vers votre tableur ou placez le fichier dans assets sans le commiter.',
    ].join('\n'),
  },
};

const triggerPlaceholderDownload = (config) => {
  const blob = new Blob([config.content], { type: config.mime });
  const url = URL.createObjectURL(blob);
  const tempLink = document.createElement('a');
  tempLink.href = url;
  tempLink.download = config.filename;
...
downloadButtons.forEach((button) => {
  button.addEventListener('click', () => {
    ...
    const config = placeholderFiles[key];
    if (!config) return;
    triggerPlaceholderDownload(config);
  });
});
